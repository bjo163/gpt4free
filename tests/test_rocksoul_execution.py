...
class RocksoulExecutionTests(unittest.TestCase):
    ...
    def test_success_stops_after_first_provider(self) -> None:
        client = FakeClient()
        result = ExecutionEngine(self.db, client).execute(ExecutionRequest(model="demo", messages="hello"))
        self.assertTrue(result.ok)
    ...
    def test_policy_taxonomy(self) -> None:
        ...
        for message, error_class, action in cases:
            decision = ExecutionPolicy.decide(RuntimeError(message))
    ...
    def test_backoff_is_bounded_and_timeout_is_immediate(self) -> None:
        ...
        self.assertEqual(ExecutionPolicy.backoff_seconds(rate_limit, 0, maximum=8.0), 0.0)
    ...
    def test_provider_enters_legacy_health_cooldown_after_failure_streak(self) -> None:
        ...
        self.assertGreater(self.db.health("A").cooldown_until, 0.0)
        self.assertEqual(self.db.route_candidates("demo")[0].provider, "B")
        self.assertEqual(ProviderControlStore(self.db).get("A").state, "QUARANTINED")

    def test_total_time_budget_covers_entire_request_lifecycle(self) -> None:
        client = FakeClient({"A": [ConnectionError("network down")]})
        result = ExecutionEngine(self.db, client).execute(ExecutionRequest(model="demo", messages=[], max_attempts=3, max_total_time=0.000001))
