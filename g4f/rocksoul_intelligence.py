...
class CapabilityVerifier:
    ...
    def verify(self, provider: str, model: str, capability: str, timeout: float = 30.0) -> CapabilityEvidence:
        ...
        started = time.perf_counter(); ok = False; detail = ""
        handler: Any | None = None
        try:
            handler = self._provider(provider)
            declared = self._declared(handler, capability)
            if capability == "streaming":
                result = self.live.probe(provider, model, "stream", timeout)
                ok, detail = result.ok, "stream produced at least one chunk" if result.ok else (result.error or "stream validation failed")
            else:
                client = self._client(handler)
                kwargs: dict[str, Any] = {"model": model, "messages": [{"role": "user", "content": "Reply with exactly: ROCKSOUL_CAPABILITY_OK"}], "provider": handler, "stream": False, "timeout": max(1.0, float(timeout))}
                if capability == "tools":
                    ...
                    kwargs["messages"] = [{"role": "user", "content": [{"type": "text", "text": "Describe this image in one word."}, {"type": "image_url", "image_url": {"url": "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII="}}]}]
                elif capability == "image":
                    image = client.images.generate(model=model, prompt="A tiny abstract ROCKSOUL capability probe image", provider=handler)
                    ok, detail = bool(image), "image generation returned a response" if image else "empty image response"
                if capability != "image":
                    result = client.chat.completions.create(**kwargs)
                    if capability == "tools":
                        ok = bool(getattr(result, "tool_calls", None)); detail = "tool call returned" if ok else "no tool call returned"
                    elif capability == "structured_output":
                        try:
            ...
            declared = self._declared(handler, capability) if handler is not None else None
        latency = (time.perf_counter() - started) * 1000.0
        self.db.set_capability(provider, capability, declared, ok, ok, model=model)
...
class ExplainableRouter:
    ...
    def explain(self, model: str, requirements: Sequence[CapabilityRequirement] = (), providers: Sequence[str] | None = None, verified_only: bool = False) -> list[dict[str, Any]]:
        ...
        for candidate in self.db.route_candidates(model, providers, verified_only):
            ...
            reasons.append("model verified" if candidate.model_verified else "model not yet verified")
            ...
            output.append({"provider":candidate.provider,"accepted":accepted,"score":round(candidate.score,2),"model_verified":candidate.model_verified,"health_score":round(candidate.health_score,2),"avg_latency_ms":candidate.avg_latency_ms,"capabilities":states,"reasons":reasons})
        return output
...
class RecoveryManager:
    ...
    def quarantined(self) -> list[str]:
        with self.db.connect() as conn: names=[r["name"] for r in conn.execute("SELECT name FROM providers ORDER BY name").fetchall()]
        return [name for name in names if self.db.health(name).consecutive_failures >= 3]
    def recover(self, providers: Sequence[str] | None = None, model: str = "gpt-4o-mini", timeout: float = 30.0) -> list[dict[str, Any]]:
        ...
        for provider in targets:
            result=probe.probe(provider,model,"smoke",timeout); health=self.db.health(provider)
            results.append({"provider":provider,"probe_ok":result.ok,"latency_ms":result.latency_ms,"health_score":round(health.score,2),"re_admitted":result.ok,"status":"RE_ADMITTED" if result.ok else "QUARANTINED"})
        return results
