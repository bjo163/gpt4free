...
class CapabilityVerifier:
    ...
    def verify(self, provider: str, model: str, capability: str, timeout: float = 30.0) -> CapabilityEvidence:
        ...
        started = time.perf_counter(); ok = False; detail = ""; declared: bool | None = None
...
class ExplainableRouter:
    ...
    def explain(self, model: str, requirements: Sequence[CapabilityRequirement] = (), providers: Sequence[str] | None = None, verified_only: bool = False) -> list[dict[str, Any]]:
        ...
        for candidate in self.db.route_candidates(model, providers, verified_only):
            control = self.control.get(candidate.provider)
            states={}; reasons=[]; accepted=not control.blocked
...
class RecoveryManager:
    ...
    def quarantined(self) -> list[str]:
        return [item.provider for item in self.control.list() if item.state == "QUARANTINED"]
    def recover(self, providers: Sequence[str] | None = None, model: str = "gpt-4o-mini", timeout: float = 30.0) -> list[dict[str, Any]]:
        ...
        for provider in targets:
            ...
            self.control.begin_recovery(provider)
            ...
            state=self.control.get(provider)
            results.append({"provider":provider,"probe_ok":result.ok,"latency_ms":result.latency_ms,"health_score":round(health.score,2),"re_admitted":result.ok,"status":state.state})
        return results
