...
"""CLI adapter for the ROCKSOUL intelligence and execution planes."""
...
def inspect_provider(db: RocksoulDB, name: str) -> dict[str, Any]:
    ...
    try:
        ...
        for model in models:
            ...
            if model_name:
                db.bind_model(name, model_name, False); result["models"] += 1
        capability_map = {"streaming": getattr(provider, "supports_stream", None), "vision": getattr(provider, "supports_vision", getattr(provider, "vision", None)), "tools": getattr(provider, "supports_native_tools", None), "structured_output": getattr(provider, "supports_structured_output", None), "image": getattr(provider, "supports_image_generation", None), "audio": getattr(provider, "supports_audio", None), "video": getattr(provider, "supports_video", None)}
        for capability, declared in capability_map.items():
            if declared is not None:
                db.set_capability(name, capability, bool(declared), None, False); result["capabilities"] += 1
...
def json_health(db: RocksoulDB, provider: str | None) -> Any:
    control = ProviderControlStore(db)
    if provider:
        item = db.health(provider); state = control.get(provider)
        status = state.state if state.state != "DEGRADED" or state.state_until > time.time() else "ACTIVE"
        return {"provider": item.provider, "attempts": item.attempts, "successes": item.successes, "failures": item.failures, "success_rate": round(item.success_rate * 100.0, 2), "avg_latency_ms": item.avg_latency_ms, "p95_latency_ms": item.p95_latency_ms, "score": round(item.score, 2), "status": status, "control_reason": state.reason, "state_until": state.state_until, "last_error_class": item.last_error_class}
    return [{"provider": item.provider, "score": round((h := db.health(item.provider)).score, 2), "attempts": h.attempts, "success_rate": round(h.success_rate * 100.0, 2), "avg_latency_ms": h.avg_latency_ms, "status": control.get(item.provider).state, "control_reason": control.get(item.provider).reason} for item in control.list()]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ROCKSOUL intelligence and execution control plane")
    ...
    quarantine = sub.add_parser("quarantine"); quarantine.add_argument("provider"); quarantine.add_argument("--reason", default="manual_quarantine")
    ...
    execute = sub.add_parser("execute"); execute.add_argument("model"); execute.add_argument("message"); execute.add_argument("--provider", action="append"); execute.add_argument("--capability", action="append"); execute.add_argument("--max-attempts", type=int, default=3); execute.add_argument("--max-same-provider-attempts", type=int, default=1); execute.add_argument("--timeout", type=float, default=30.0); execute.add_argument("--total-time", type=float, default=90.0)
    trace = sub.add_parser("trace"); trace.add_argument("request_id")
    sub.add_parser("status")
    args = parser.parse_args(); db = RocksoulDB(); execution_store = ExecutionTraceStore(db); control = ProviderControlStore(db)

    if args.command == "discover": print(json.dumps(discover_all(db), indent=2))
    elif args.command == "migrate": print(json.dumps({"migrated": db.migrate_legacy_json(), "db": str(db.path)}, indent=2))
    elif args.command == "provider": print(json.dumps(inspect_provider(db, args.name), indent=2))
    elif args.command == "health": print(json.dumps(json_health(db, args.provider), indent=2))
    ...
    elif args.command == "route-explain": print(json.dumps(ExplainableRouter(db).explain(args.model, _requirements(args)), indent=2))
    elif args.command == "probe":
        model = _model_for_provider(db, args.provider, args.model); print(json.dumps(LiveProbe(db).probe(args.provider, model, args.type, args.timeout).to_dict(), indent=2))
    elif args.command == "verify":
        model = _model_for_provider(db, args.provider, args.model); results = CapabilityVerifier(db).verify_many(args.provider, model, args.capability or ["streaming"], args.timeout); print(json.dumps([item.to_dict() for item in results], indent=2))
    elif args.command == "probe-all":
        with db.connect() as conn: providers = [row["name"] for row in conn.execute("SELECT name FROM providers ORDER BY name").fetchall()]
        if not providers: discover_all(db); providers = [item.provider for item in control.list()]
        model = args.model or default_probe_model(); results = LiveProbe(db).probe_many(providers, model, args.concurrency, args.type, args.timeout); print(json.dumps(result_summary(results), indent=2))
    elif args.command == "quarantine": print(json.dumps(asdict(control.quarantine(args.provider, args.reason)), indent=2))
    elif args.command == "recover": print(json.dumps(RecoveryManager(db).recover(args.provider, args.model, args.timeout), indent=2))
    elif args.command == "execute":
        request = ExecutionRequest(model=args.model, messages=({"role": "user", "content": args.message},), requirements=tuple(args.capability or ()), providers=tuple(args.provider or ()), max_attempts=max(1, args.max_attempts), max_same_provider_attempts=max(1, args.max_same_provider_attempts), timeout=max(0.1, args.timeout), max_total_time=max(0.1, args.total_time))
    ...
    elif args.command == "trace": print(json.dumps(execution_store.trace(args.request_id) or {"request_id": args.request_id, "found": False}, indent=2))
    elif args.command == "status":
        with db.connect() as conn:
            counts = {key: int(conn.execute(query).fetchone()[0]) for key, query in {"providers":"SELECT COUNT(*) FROM providers","models":"SELECT COUNT(*) FROM models","provider_models":"SELECT COUNT(*) FROM provider_models","capabilities":"SELECT COUNT(*) FROM capabilities","probes":"SELECT COUNT(*) FROM probe_runs","route_decisions":"SELECT COUNT(*) FROM route_decisions","execution_runs":"SELECT COUNT(*) FROM execution_runs","execution_attempts":"SELECT COUNT(*) FROM execution_attempts","control_events":"SELECT COUNT(*) FROM provider_control_events"}.items()}
        print(json.dumps({"db": str(DB_PATH), **counts, "provider_states": [asdict(item) for item in control.list()]}, indent=2, default=str))
